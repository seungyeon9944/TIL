...
### 오버피팅 (overfitting)⭐️
훈련 데이터의 **우연한 패턴/잡음**까지 외워버려서 훈련에서는 잘 맞지만 테스트에서는 성능이 나빠지는 현상 → **훈련 오류 급격히 낮음** 현상, 테스트 오류 **높음/요동** 현상
- **표본(sample) 의존/불안정** : 훈련 데이터는 모집단의 일부 표본. 과하게 맞추어 학습하면 샘플 몇 개만 바뀌어도 예측이 크게 흔들림
- **일반화 실패** : 보지못한 데이터(테스트) 오류가 커짐, 모집단 성능과 격차 벌어짐
오버피팅은 분포 변화로 인한 에러 증가와 다르다 !

### 언더피팅
모델이 단순하거나 학습이 완료되지않음 → 중요한 패턴 놓침 (오류 큼)

---
### 훈련 오류
모델을 학습시킨 **같은 데이터**에 다시 적용해 계산한 오류
### 테스트 오류
학습에 쓰지않은 **새 관측치**에 대해 모델을 적용했을 때 평균 예측오류

sol) **재표본화(resampling)** 통한 테스트 오류 추정
- **데이터를 나눠** 여러 번 "훈련 → 평가"를 반복해 테스트 오류 가늠
- 방법 : 검증셋(hold-out), K겹 교차검증(K-fold Cross-Validation)
- 별도 테스트 데이터 없이 데이터 효율적 사용 가능

가용 샘플들을 무작위로 **훈련셋**(→ **모델 적합**)으로 학습 / **검증셋(hold-out)**(→ **예측 후 검증 오류**)으로 성능평가

---

### K-겹 교차검증 (K-fold Cross-Validation)
데이터를 먼저 셔플링한 뒤 총 n개 데이터를 **겹치지않는 (mutually exclusive) K개 그룹으로 분할**하여 검증셋과 훈련셋을 번갈아 분할, K개의 MSE를 평균해 테스트 오류 추정
...
### Leave-One-Out 교차검증
훈련셋을 관측치 하나만 제외한 나머지 전부, 검증셋을 제외한 1개 관측치로 하여 이 과정을 n번 반복해 MSE 평균으로 테스트 오류 추정

---

## 비지도학습
레이블(정답) 없이 데이터의 구조/패턴/집단(잠재 서브그룹) 찾아내는 학습 → 군집화(clustering), 차원축소(PCA 등), 밀도추정/이상치 탐지.
지도학습과 다르게 입력만으로 구조 학습

### 클러스터링 (Clustering)
데이터 안에서 하위집단(클러스터)을 찾는 기법들. 집단 내부는 서로 유사, 집단 간은 상이하도록
### 1. K-평균(K-means)
K(클러스터 수) **미리 정해** 분할
- 좋은 군집화 = **클러스터 내부 변동** (Within-Cluster Variation)이 작은 분할
- 목표 : **클러스터 내부 변동의 합이 최소**가 되도록 분할 찾음
- **초기화** (관측치에 무작위로 1, ... , K 클러스터 임시 부여) → **반복** (각 클러스터의 **중심 centroid** 계산, 각 관측치를 **가장 가까운 중심**의 클러스터에 **재할당**)
- **목표함수 값 감소** but 전역 최솟값 (global) 보장은 아님

### 2. 계층적 군집(Hierarchichal Clustering)
K를 고정하지 않고 전체 구조를 덴드로그램으로 제공. 
...
### 최소제곱법 (least squares)
**실제 관측값과 에측값의 차이**를 제곱해 합한 값(**잔차제곱합**)을 최소화하는 방법
$$
e_i = y_i - \hat{y}_i = y_i - (\beta_0 + \beta_1 x_{i1})
$$
